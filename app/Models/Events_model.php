...
protected $allowedFields = [
        ...
        'created_by','location','client_id','labels','share_with','editable_google_event',
        'google_event_id','deleted','lead_id','ticket_id','project_id','task_id',
        'proposal_id','contract_id','subscription_id','invoice_id','order_id','estimate_id',
        'next_recurring_time','no_of_cycles_completed','snoozing_time','reminder_status',
        'type','color','recurring','repeat_every','repeat_type','no_of_cycles','last_start_date',
        'recurring_dates','confirmed_by','rejected_by','files','ingesta','asi_ip','conv','ig',
        'extra','smd','smd_color','ig_color','conv_color','asi_ip_color','extra_bg_color','extra_text_color',
        // nuevos
        'import_source','import_batch_id','imported_at',
];
    ...
    // Método para insertar eventos rápidos
    ...
    function get_details($options = array()) {
        ...
        $sql = "SELECT $events_table.*, rise_custom_fields.options AS asi_ip_options, 
        CONCAT($users_table.first_name, ' ', $users_table.last_name) AS created_by_name,
        $users_table.image AS created_by_avatar, $clients_table.company_name, $clients_table.is_lead, $select_labels_data_query
    FROM $events_table
    ...
    LEFT JOIN rise_custom_fields ON rise_custom_fields.related_to = 'events' AND rise_custom_fields.title = 'ai_ip'
    WHERE $events_table.deleted=0 $where
    $order_by
    LIMIT $offset, $limit";  // Asegúrate de no poner comillas alrededor de $offset y $limit

$query = $this->db->query($sql);  // Asigna el resultado de la consulta a la variable $query

if (!$query) {
    $dbError = $this->db->error(); // Captura el error como array
    die("MySQL Error: " . $dbError['message']); // Muestra solo el mensaje de error
}
...
return $query;
    ...
    public function get_search_suggestion($search) {
        ...
        return $this->db->table($events_table)
        ...
        /**
     * Soft-delete por día. $scope: 'csv' (solo importados) | 'all' (todos)
     * Retorna cantidad afectada.
     */
    public function softDeleteByDay(string $date, string $scope='csv'): int
    {
        $b = $this->db->table($this->table);
        $b->set('deleted', 1)
          ->where('deleted', 0)
          ->where('start_date', $date); // tu start_date es DATE
...
public function get_resource_usage($filters = [])
{
    ...
    // --- MODO 1: datetime puntual ---
    if (!empty($filters["datetime"])) {
        $dt = $filters["datetime"];
        $params[] = $dt; // inicio debe ser <= datetime
        $params[] = $dt; // fin debe ser >= datetime

        $conditions[] = "CONCAT(start_date,' ',IFNULL(start_time,'00:00:00')) <= ?";
        $conditions[] = "(
            (end_date IS NULL AND (end_time IS NULL OR end_time='00:00:00'))
            OR CONCAT(IFNULL(end_date,start_date),' ',
                IFNULL(NULLIF(end_time,'00:00:00'),'23:59:59')
            ) >= ?
        )";
    }
    ...
    if (!empty($filters["resource"])) {
        $r = $filters["resource"];
        if (in_array($r, ["smd","conv","ig","asi_ip"])) {
            $conditions[] = "$r IS NOT NULL AND $r != ''";
        }
    }
    ...
    $sql = "
        SELECT 
            ...
            SUM(CASE WHEN smd IS NOT NULL AND smd != '' THEN 1 ELSE 0 END) AS total_smd,
            SUM(CASE WHEN conv IS NOT NULL AND conv != '' THEN 1 ELSE 0 END) AS total_conv,
            SUM(CASE WHEN ig IS NOT NULL AND ig != '' THEN 1 ELSE 0 END) AS total_ig,
            SUM(CASE WHEN asi_ip IS NOT NULL AND asi_ip != '' THEN 1 ELSE 0 END) AS total_asi_ip,
            GROUP_CONCAT(DISTINCT smd) AS lista_smd,
            GROUP_CONCAT(DISTINCT conv) AS lista_conv,
            GROUP_CONCAT(DISTINCT ig) AS lista_ig,
            GROUP_CONCAT(DISTINCT asi_ip) AS lista_asi_ip
        FROM rise_events
        WHERE deleted = 0
    ";
    ...
    // ---- Clasificación de ASI-IP por marcas ----
    $asips = array_filter(array_map("trim", explode(",", $row["lista_asi_ip"] ?? "")));
    ...
    foreach ($asips as $asi) {
        if (preg_match("/^IRD\s*\d+-\d+/i", $asi)) {
            $marcas["MediaKind"]++;
        } elseif (preg_match("/^IRD\s*\d+$/i", $asi)) {
            $marcas["ATEME"]++;
        } elseif (preg_match("/^(SRT|RX SRT)\s*\d+/i", $asi)) {
            $marcas["Nimbra/Kiloview"]++;
        } elseif ($asi !== "") {
            $marcas["Otros"]++;
        }
    }

    $row["asiip_marcas"] = $marcas;
    ...
    /**
     * Revertir (restaurar) un lote completo por import_batch_id (opcional).
     */
    public function restoreByBatch(string $batchId): int
    {
        $b = $this->db->table($this->table);
        $b->set('deleted', 0)
