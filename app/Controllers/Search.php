...
class Search extends Security_Controller {
    ...
    function get_search_suggestion() {
        ...
        if ($search && $search_field) {
            ...
            else if ($search_field == "event") {
                $result = $this->Events_model->get_search_suggestion($search)->getResult();
            }
            ...
            foreach ($result as $value) {
                if ($search_field == "task") {
                    $result_array[] = array(
                        "value" => $value->id,
                        "label" => app_lang("task") . " $value->id: " . $value->title
                    );
                } else if ($search_field == "event") {
                    $start_datetime = format_to_datetime($value->start_date . ' ' . $value->start_time, false);
                    $result_array[] = array(
                        "value" => encode_id($value->id, "event_id"),
                        "label" => $value->title . ' <span style="color:#888;font-size:0.8em;"> - ' . $start_datetime . '</span>'
                    );                                       
                } else {
                    $result_array[] = array(
                        "value" => $value->id,
