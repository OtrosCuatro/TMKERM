...
<?php echo modal_anchor(get_uri("events/view"), "", array(
    "class" => "hide",
    "id" => "show_event_hidden",
    "title" => app_lang('event_details'),
    "data-post-id" => "" // esto se sobrescribe luego con JS
)); ?>
...
// Sobrescribe el método de generación de ítems para permitir HTML
awesomplete.item = function (text, input) {
    var html = (typeof text === "object" && text.label) ? text.label : text;
    ...
    return Awesomplete.$.create("li", {
        innerHTML: text.label || text,  // no usa `textContent`, así permite HTML
        "aria-selected": "false"
    });
};
    ...
    if (this.value) {
        ...
        } else if (searchFieldValue === "event") {
    $("#show_event_hidden")
        .attr("data-post-id", this.value)
        .trigger("click");
} else {
